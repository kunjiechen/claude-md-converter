...
Historical extraction note: the following were target-spec gaps when the V2
plan was drafted:
...
## 14. Phase 9 Production Hardening Addendum

Current V2 implementation status:

- Typed DocumentModel exists as the normalized representation.
- SemanticAnalysis, RenderPolicy, and LayoutPlan exist as renderer-independent stages.
- HTML/DOCX/PDF RendererAdapter implementations are available behind flags or the unified entrypoint.
- Legacy renderers remain the default production fallback.
- Unified rendering reports must include diagnostics, fidelity, render trace, performance timings, and quality gate status.
- `render-rules.yaml` must fail early on invalid profile, renderer policy, backend, fidelity, or unsafe unsupported-content behavior.

Production constraints:

- Do not default-enable unified adapter pipeline without rollout approval.
- Do not remove legacy patch/polish paths until retirement conditions are satisfied.
- Do not allow renderers to classify table kind, infer document profile, or invent overflow strategy.
- All fallback and degradation behavior must generate diagnostics and affect quality gate status.

## 15. Phase 10 Release Cutover Addendum

Default rollout matrix:
...
- `legacy_only`: never select V2 automatically.
- `v2_canary`: select V2 only for governed canary rollout.
- `v2_default_with_fallback`: select V2 where the matrix permits it and record fallback if legacy is used after failure.
- `v2_strict`: fail before rendering when matrix/readiness does not permit V2.

CLI/API policy:

- `pipeline=legacy` forces legacy.
- `pipeline=v2` forces V2 with fallback unless strict mode disables fallback.
- `pipeline=auto` uses the rollout selector.
- formal PDF must not default to V2.
- all fallback must be visible in `UnifiedRenderReport`.

## 16. Phase 11 Legacy Deletion Review Addendum

Legacy deletion is governed by classification, not by availability of a V2
implementation alone.

Deletion classifications:

- `safe_to_remove`: fully replaced, not used, covered, rollback exists.
- `remove_after_rollout`: V2 replacement exists but rollout/fallback metrics are not complete.
- `compatibility_required`: must remain until equivalent independent validation exists.
- `renderer_specific_required`: target-format quirk; keep inside adapter or legacy boundary.
- `unknown_risk`: insufficient coverage or unclear replacement.

Current deletion posture:

- no legacy area is classified as immediately safe to remove;
- HTML legacy renderer is the first future candidate after rollout evidence;
- DOCX/PDF legacy paths remain profile/backend dependent;
- preflight, postflight, and compatibility quirks are retention candidates;
- `polisher.py` must be retired patch-by-patch only.

## 17. Phase 12 Maintenance Mode Addendum

Architecture freeze:

- `DocumentModel`, `SemanticAnalysis`, `RenderPolicy`, `LayoutPlan`,
  `RendererAdapter`, and `UnifiedRenderReport` are stable public boundaries.
- Renderer-specific units remain adapter concerns.
- Profiles and plugins extend behavior without changing core pipeline code.

Versioned surfaces:

- `schema_versions.spec_version`
- `schema_versions.render_rules_schema`
- `schema_versions.profile_schema`
- `schema_versions.adapter_capability_schema`
- `PluginInfo.capability_version`

Plugin extension points:

- table classifier;
- document profile analyzer;
- renderer adapter;
- asset resolver;
- diagram renderer;
- quality gate.
...
- new profiles live in `config/profiles/*.yaml`;
- profile YAML may `extends` a base profile;
- profile overrides are deep-merged;
- unsupported content policy must remain diagnostic-backed.

Maintenance policy:

- no large refactors in maintenance mode;
- default legacy behavior remains unchanged;
- deprecations require migration documentation and one release window;
- golden updates require explicit reason and review.

## 18. Real Document Validation Addendum

Default V2 rollout must be supported by real-document fidelity evidence, not
only unit tests.

Validation corpus categories:

- chip/register manuals;
...
Initial real-document finding:

- `G-C110 流程图编制规范_A0.md` renders successfully through legacy and V2 for HTML/DOCX/PDF.
- V2 does not fallback for this sample.
- V2 remains review fidelity because the document contains raw HTML tables/images, TOC/revision concerns, and many semantic diagnostics.
- DOCX/PDF should not default to V2 for `requirement_spec` based on one document.

Rollout evidence requirement:

- at least 10 representative real documents per target profile;
- fallback rate below 1%;
- degraded rate below 5%;
- no silent content loss;
- manual signoff for TOC/revision/table-heavy formal documents.
